...
class Arnold_Liouville(nn.Module):
    ...
    def __init__(self, input_size):
        ...
        self.Actor = MLP(int(input_size / 2), int(input_size / 2))
    ...
    def optimize(self, batch):
        ...
        psi_loss = 1 * (torch.mean((x - x_hat)**2) + torch.mean((x1 - x1_hat)**2) + torch.mean((x1 - x1_dyn_hat)**2))
        ...
        self.opt.zero_grad()
        (psi_loss+phi_loss+phi_loss_neg).backward()
        self.opt.step()
